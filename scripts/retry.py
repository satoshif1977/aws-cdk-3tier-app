...
EC2 の RequestLimitExceeded や ELBv2 / RDS の Throttling のように
「時間をおけば成功しうる」失敗に対して、AWS 公式推奨の
「指数バックオフ + フルジッター」方式で自動リトライする。

verify_stack.py は describe 系 API を短時間に連続で呼ぶため、
アカウント単位の API レート制限に当たりやすい。1 回のスロットリングで
検証全体が落ちると、デプロイが成功しているのに失敗と判定されてしまう。

設計方針:
  - sleep / rand を注入可能にして、テストを決定的かつ実待機ゼロに保つ
  - リトライ不能な例外（AccessDenied / ValidationError 等）は即座に再送出する
  - 最終試行でも失敗した場合は元の例外をそのまま再送出する
    （呼び出し側の `except ClientError` を壊さないため、独自例外でラップしない）

使い方:
    vpcs = retry_call(ec2_client.describe_vpcs, Filters=[...])["Vpcs"]
"""
